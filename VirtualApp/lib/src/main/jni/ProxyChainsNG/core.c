...
//        PDEBUG(LOG_PREFIX "[EROOR][android] socket() failed: %s (socket fd = %d)\n", strerror(errno), sock);
//        return -1;
//    }
//
//    // Set timeout to 1s
...
//        PDEBUG(LOG_PREFIX "[EROOR][android] connect() failed: %s (socket fd = %d), path: %s\n",
...
static int tunnel_to(int sock, ip_type ip, unsigned short port, proxy_type pt, char *user, char *pass) {
    ...
    if (ulen > 0xFF || passlen > 0xFF || dns_len > 0xFF) {
        PDEBUG(LOG_PREFIX
                       "error: maximum size of 255 for user/pass or domain name!\n");
        goto err;
    }
...
#define HTTP_AUTH_MAX ((0xFF * 2) + 1 + 1) /* 2 * 0xff: username and pass, plus 1 for ':' and 1 for zero terminator. */
        ...
        case SOCKS4_TYPE: {
            if (v6) {
                PDEBUG(LOG_PREFIX
                               "error: SOCKS4 doesn't support ipv6 addresses\n");
                goto err;
            }
...
static int start_chain(int *fd, proxy_data *pd, char *begin_mark) {
    ...
    *fd = socket(v6 ? PF_INET6 : PF_INET, SOCK_STREAM, 0);
    ...
    if (timed_connect(*fd, (struct sockaddr *) (v6 ? (void *) &addr6 : (void *) &addr),
                      v6 ? sizeof(addr6) : sizeof(addr))) {
        pd->ps = DOWN_STATE;
        goto error1;
    }
    pd->ps = BUSY_STATE;
...
static int chain_step(int ns, proxy_data *pfrom, proxy_data *pto) {
    ...
    switch (retcode) {
        ...
        case SOCKET_ERROR:
            pto->ps = DOWN_STATE;
            PDEBUG("<--socket error or timeout!\n");
            close(ns);
            break;
    }
    return retcode;
